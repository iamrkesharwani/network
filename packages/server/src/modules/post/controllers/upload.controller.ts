...
export const initiateTheVideoUpload = asyncHandler(
  async (req: Request, res: Response) => {
    if (!req.user)
      throw new ApiError(401, 'UNAUTHORIZED', 'Authentication required.');
    ...
    const result = await initiateVideoUpload(
      req.user.id,
      req.body as InitiatePostVideoUploadInput
    );
...
export const confirmTheVideoUpload = asyncHandler(
  async (req: Request, res: Response) => {
    if (!req.user)
      throw new ApiError(401, 'UNAUTHORIZED', 'Authentication required.');

    const { postId, storageKey, fileSizeBytes } =
      req.body as ConfirmPostVideoUploadInput;

    const post = await confirmVideoUpload(
      req.user.id,
      postId,
      storageKey,
      fileSizeBytes
    ...
    res
      .status(200)
      .json(new ApiResponse(post, 'Upload confirmed. Processing started.'));
  }
);
