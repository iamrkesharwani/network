...
const BorderedTextarea = forwardRef<
  HTMLTextAreaElement,
  BaseProps & React.TextareaHTMLAttributes<HTMLTextAreaElement>
>(({ label, icon: Icon, error, hint, containerClassName, counter, className, ...props }, ref) => (
  <div className={cn('mb-6', containerClassName)}>
    ...
    <textarea
      ...
      aria-invalid={!!error}
      className={cn(
        'w-full resize-none rounded-lg border bg-surface-raised px-3.5 py-2.5 text-sm font-medium text-text-primary outline-none transition-colors',
    ...
    <div className="mt-1.5 flex items-start justify-between gap-3">
      <div className="min-w-0">
        ...
        {hint && !error && <p className="text-[0.72rem] text-text-muted">{hint}</p>}
      </div>
