...
const MediaDropzone = ({
  ...
  if (resumePointer && state.stage === 'idle') {
    const percent =
      resumePointer.totalParts > 0
        ? Math.round(
            (resumePointer.uploadedParts.length / resumePointer.totalParts) *
              100
          )
        : 0;

    return (
      <div className="flex flex-col items-center justify-center min-h-64 gap-4 rounded-2xl border-2 border-dashed border-primary/40 bg-primary-subtle px-6 py-10 text-center">
        ...
        <div>
          ...
          <p className="mt-0.5 text-xs text-text-muted">
            {percent}% already uploaded — select the same file to continue.
          </p>
        </div>
