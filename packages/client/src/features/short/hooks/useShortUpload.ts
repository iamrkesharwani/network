...
export const useRawShortUpload = () => {
  ...
  const startUpload = useCallback(
    async (file: File) => {
      const validationError = validateShortFile(file);
      if (validationError) {
      ...
      if (durationSeconds && durationSeconds > MAX_SHORT_DURATION_SECONDS) {
        setState({
          ...initialState,
          stage: 'error',
          file,
          error: 'That video is too long. Shorts must be under 60 seconds.',
        });
        return;
      }
      ...
      try {
        const initResult = await initiateUpload({
          ...
          mimeType: file.type as (typeof ALLOWED_SHORT_MIME_TYPES)[number],
          ...(durationSeconds !== undefined && { durationSeconds }),
        }).unwrap();

        shortId = initResult.data.shortId;
      ...
      setState((prev) => ({ ...prev, stage: 'uploading', videoId: shortId }));
  ...
  const cancelUpload = useCallback(() => {
    ...
    const { videoId: shortId } = state;
    if (shortId) {
      deleteShort(shortId).catch(() => undefined);
    }
    reset();
  }, [deleteShort, reset, state]);
